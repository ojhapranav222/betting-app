...
function Card({ country1, country2, point1, point2, isLive, startingIn, type }) {
  
    return (
      <SpotlightCard className='border border-gray-400 bg-gray-950'>
        <div className="text-white">
            <div className='flex justify-between'>
                <h2 className="font-bold bg-gray-900 rounded-xl text-gray-400 px-6 py-1">{type}</h2>
                <div>
                    {isLive ? (
                        <span className="text-red-500 font-bold flex items-center gap-2 text-sm"><CgMediaLive />LIVE</span>
                    ) : (
                        <span className="text-gray-400 flex items-center gap-2 text-sm">
                           <IoIosTimer /> Starting in: {startingIn}
                        </span>
                    )}
                </div>
            </div>
